...
- There are many ways to subset a data frame. Some of them are *[]* operator, *$* operator, or *subset()* function
...
- *[]* operator requres two arguments - one for rows (observations) and one for columns (variables) - *[rows, columns]*
...
- Logical operators, such as ==, !=, >, >=, <, <=, &, | can be also used for indexing
...
- Basic functions of **dplyr** package are *select*, *arrange*, *filter*, *mutate*, and *summarize*
...
- *group_by* do not change data. Its role is to create the groups of variables to be use by the previous functions
...
- **tidyr** package has two main functions - *gather* and *spread*, and several additional functions

---
## **tidyr** - *spread*

- *spread* function takes two columns (key & value) and spreads in to multiple columns (creates data in "wide" format)
...
- *gather* function reshape data from wide format to long format
...
- The usual approach to perform multiply operations in R is either by creating many "subobjects" or by nesting
...
- Nowadays, it could be easier with the use of *%>%* ("pipe") operator
- In this approach, commands are written in a natural order
